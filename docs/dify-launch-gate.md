...
does not require `dify-aio.xml` in `awesome-unraid` yet. The icon can exist in
the catalog repo before the XML.
...
- the catalog PR only syncs the XML once the source repo is ready.
