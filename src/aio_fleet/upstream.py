...
def create_or_update_upstream_pr(
    ...
    *,
    ...
    run_git(repo.path, ["config", "user.name", "github-actions[bot]"])
    run_git(
        repo.path,
        [
            "config",
    ...
    run_git(repo.path, ["checkout", "-B", branch, "origin/main"])
    run_git(repo.path, ["add", *commit_paths])
    diff = run_git(repo.path, ["diff", "--cached", "--quiet"], check=False)
    if diff.returncode == 0:
        return {"repo": repo.name, "action": "skipped", "reason": "no-diff"}
    run_git(repo.path, ["commit", "-m", title])
    run_git(repo.path, ["push", "-u", "origin", branch])
    pr_url = upsert_pr(repo, branch=branch, title=title, body=body)
    head = run_git(repo.path, ["rev-parse", "HEAD"]).stdout.strip()
    if post_check:
        upsert_check_run(
            repo,
            sha=head,
    ...
    return {"repo": repo.name, "action": "upserted-pr", "branch": branch, "url": pr_url}
